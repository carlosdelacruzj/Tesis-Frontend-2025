...
type UbicacionRowEditable = UbicacionRow & {
  _backup?: UbicacionRow;
  editing?: boolean;
  _fechaPrev?: string;
};
...
export class ActualizarPedidoComponent implements OnInit, AfterViewInit {
  ...
  readonly fechaMinimaEvento =
    ActualizarPedidoComponent.computeFechaMinimaEvento();
  readonly fechaMaximaEvento =
    ActualizarPedidoComponent.computeFechaMaximaEvento();
  ...
  dataSource: MatTableDataSource<UbicacionRow> =
    new MatTableDataSource<UbicacionRow>([]);
  ...
  private asegurarDepartamentoEnCatalogo(value: unknown): string {
    ...
    const existente = this.departamentos.find(
      (dep) => this.normalizarDepartamento(dep) === normalizado,
    );
    if (existente) {
      return existente;
    }
  ...
  ngOnInit(): void {
    ...
    this.visualizarService.selectAgregarPedido.fechaCreate = formatDisplayDate(
      this.fechaCreate,
      '',
    );
    ...
    this.lastDepartamento = (
      this.visualizarService.selectAgregarPedido.departamento ?? ''
    )
      .toString()
      .trim();
  }
  ...
  private getPkgKey(el: unknown): string {
    ...
    const rawId =
      record['idEventoServicio'] ?? eventoServicio['id'] ?? record['id'];
    ...
    obs
      ...
      .subscribe((res: unknown) => {
        ...
        this.clienteNombreCompleto =
          razonSocial || [nombres, apellidos].filter(Boolean).join(' ').trim();
  ...
  getEventoxServicio() {
    const obs = this.visualizarService?.getEventosServicio?.(
      ...
      .subscribe((res: unknown) => {
        const activos = Array.isArray(res)
          ? res.filter((item) => {
              ...
              const estadoId =
                typeof estado?.['id'] === 'number' ? estado['id'] : null;
              return estadoNombre !== 'inactivo' && estadoId !== 2;
            })
          : [];
        this.paquetesRows = activos.map((item) =>
          this.normalizePaqueteRow(item as AnyRecord),
        );
        this.loadingPaquetes = false;
      });
  }
  ...
  hasOtroPaqueteDelServicio(element: AnyRecord): boolean {
    ...
    const servicioId =
      typeof servicio?.['id'] === 'number' ? servicio['id'] : null;
    if (servicioId == null) {
      return false;
    }
  ...
  confirmPrecioEdit(
    paquete: PedidoPaqueteSeleccionado,
  ...
  addPaquete(
    ...
    const eventoRecord = record['evento'] as
      | Record<string, unknown>
      | undefined;
    const servicioRecord = record['servicio'] as
      | Record<string, unknown>
      | undefined;
    const eventoId =
      typeof eventoRecord?.['id'] === 'number' ? eventoRecord['id'] : null;
    const servicioId =
      typeof servicioRecord?.['id'] === 'number' ? servicioRecord['id'] : null;
    ...
    const servicioNombreActual =
      typeof servicioRecord?.['nombre'] === 'string'
    ...
    const personal =
      staffRecord?.['total'] != null ? Number(staffRecord['total']) : null;
    const fotosImpresas =
      record['fotosImpresas'] != null ? Number(record['fotosImpresas']) : null;
    const trailerMin =
      record['trailerMin'] != null ? Number(record['trailerMin']) : null;
    const filmMin =
      record['filmMin'] != null ? Number(record['filmMin']) : null;
  ...
  removePaquete(
    ...
    const removed = this.selectedPaquetes.find(
      (p) => p.key === key && p.eventKey === eventoKey,
    );
    this.selectedPaquetes = this.selectedPaquetes.filter(
      (p) => !(p.key === key && p.eventKey === eventoKey),
    );
    if (removed?.tmpId) {
      this.serviciosFechasSeleccionadas =
  ...
  private getViaticosMontoTotal(): number {
    ...
    const viaticosCliente = Boolean(
      this.visualizarService.selectAgregarPedido?.viaticosCliente,
    );
    if (viaticosCliente) {
      return 0;
    }
    const monto = this.parseNumber(
      this.visualizarService.selectAgregarPedido?.viaticosMonto,
    );
  ...
  getFechaTrabajoLabel(index: number): string {
    ...
    return fecha
      ? `Dia ${index + 1} · ${this.formatFechaConDia(fecha)}`
      : `Dia ${index + 1} · Sin fecha`;
  }
  ...
  getProgramacionIndicesPorFecha(fecha: string): number[] {
    ...
    return this.ubicacion
      ...
      .filter(
        (entry) => (entry.item.Fecha ?? '').toString().trim() === objetivo,
      )
      .sort((a, b) => {
  ...
  getProgramacionDireccionResumen(index: number): string {
    const direccion = (this.ubicacion[index]?.DireccionExacta ?? '')
      .toString()
      .trim();
    if (!direccion) {
  ...
  canAgregarLocacion(fecha: string): boolean {
    ...
    return (
      !!objetivo &&
      this.getProgramacionIndicesPorFecha(objetivo).length <
  ...
  copiarLocacionesDiaAnterior(index: number, fechaDestinoParam?: string): void {
    ...
    const fechaDestino = (fechaDestinoParam ?? this.fechasTrabajo[index] ?? '')
      .toString()
      .trim();
    ...
    if (!indicesOrigen.length) {
      this.showAlert(
        'info',
        'Sin locaciones',
        'El dia anterior no tiene locaciones para copiar.',
      );
      return;
    }
    const disponibles = Math.max(
      0,
      this.maxLocacionesPorDia -
        this.getProgramacionIndicesPorFecha(fechaDestino).length,
    );
    if (!disponibles) {
      this.showAlert(
        'warning',
        'Limite alcanzado',
        `Maximo ${this.maxLocacionesPorDia} locaciones por dia.`,
      );
      return;
    }
    const aCopiar = indicesOrigen.slice(0, disponibles);
    let nextId = this.ubicacion.length
      ? Math.max(...this.ubicacion.map((u) => u.ID)) + 1
      : 1;
  ...
  addUbicacionPorDia(index: number, fechaDestinoParam?: string): void {
    const fecha = (fechaDestinoParam ?? this.fechasTrabajo[index] ?? '')
      .toString()
      .trim();
    if (!fecha) {
      this.showAlert(
        'warning',
        'Fecha requerida',
        'Primero define la fecha de este dia.',
      );
      return;
    }
  ...
  duplicarUbicacionById(id: number): void {
    ...
    const nextId = this.ubicacion.length
      ? Math.max(...this.ubicacion.map((u) => u.ID)) + 1
      : 1;
    const nueva = {
      ID: nextId,
  ...
  private remapUbicacionesFechas(prev: string[], next: string[]): void {
    ...
    for (let i = 0; i < max; i += 1) {
      ...
      this.serviciosFechasSeleccionadas = this.serviciosFechasSeleccionadas.map(
        (entry) =>
          (entry.fecha ?? '').toString().trim() === oldFecha
  ...
  onFechaTrabajoChange(index: number, value: unknown): void {
    ...
    const duplicada = this.fechasTrabajo.some(
      (item, i) =>
    ...
    if (duplicada) {
      ...
      this.showAlert(
        'warning',
        'Fecha repetida',
        'Cada dia de trabajo debe tener una fecha diferente.',
      );
      return;
    }
  ...
  onUbicacionFechaChange(row: UbicacionRowEditable, value: unknown): void {
    ...
    if (fechasUnicas.length <= maxDias) {
      ...
      if (fechaAnterior && fechaAnterior !== fecha) {
        const sigueUsada = this.ubicacion.some(
          (item) => item.Fecha === fechaAnterior,
        );
        if (
          !sigueUsada &&
          this.visualizarService.selectAgregarPedido.fechaEvent ===
    ...
    Swal.fire({
      ...
      if (
        this.visualizarService.selectAgregarPedido.fechaEvent === fechaAnterior
      ) {
        this.visualizarService.selectAgregarPedido.fechaEvent = fecha;
      }
      if (this.serviciosFechasSeleccionadas.length) {
  ...
  onQuickAdd(fechaForzada?: string): void {
    ...
    this.ubicacion = [...this.ubicacion, nueva];
    this.setProgramacionExpandidaById(nueva.ID, true);
    this.dataSource.data = this.ubicacion;
    this.bindSorts();
    this.syncProgramacionExpandida();
  ...
  addListUbicacion(
    ...
    const yaExiste = this.ubicacion.some(
      (u) =>
        u.Fecha === fecha &&
        u.Hora === hora &&
    ...
    this.ubicacion = this.ubicacion.filter((item) => {
      return (
        item.ID != cualEliminar.ID && item.Direccion != cualEliminar.Direccion
      );
    });

    const i = this.ubicacion.length
      ? Math.max(...this.ubicacion.map((u) => u.ID)) + 1
      : 1;
    const parts = this.splitHoraParts(hora);
    const nueva = {
  ...
  async deleteElement(p: string, c: string) {
    ...
    const idx = this.ubicacion.findIndex(
      (x) => x.Hora == c && x.Direccion == p,
    );
    if (idx >= 0) {
      ...
      this.showToast(
        'success',
        'Locacion eliminada',
        'Se elimino la locacion seleccionada.',
      );
    }
  }
  ...
  private loadPedido(id: number) {
    const obs = this.visualizarService.getPedidoById?.(id) as
    ...
    obs
      ...
      .subscribe((data: PedidoResponse | null) => {
        ...
        this.visualizarService.selectAgregarPedido.mensaje =
          pedido.mensaje ?? '';
        this.visualizarService.selectAgregarPedido.departamento =
          this.asegurarDepartamentoEnCatalogo(pedido.lugar ?? 'Lima');
        this.lastDepartamento =
          this.visualizarService.selectAgregarPedido.departamento
            .toString()
            .trim();
        const viaticosClienteApi =
          typeof pedido.viaticosCliente === 'boolean'
            ? pedido.viaticosCliente
            : !(pedido.viaticosMonto && pedido.viaticosMonto > 0);
        this.visualizarService.selectAgregarPedido.viaticosCliente =
          viaticosClienteApi;
        this.visualizarService.selectAgregarPedido.viaticosMonto =
          pedido.viaticosMonto ?? null;
        ...
        const fechaCreacionParsed =
          parseDateInput(pedido.fechaCreacion) ?? new Date();
        this.fechaCreate = fechaCreacionParsed;
        this.visualizarService.selectAgregarPedido.fechaCreate =
          formatDisplayDate(fechaCreacionParsed, '');
        ...
        const diasInferidos = fechasUnicas.size
          ? fechasUnicas.size
          : pedido.fechaEvento
            ? 1
            : null;
        this.visualizarService.selectAgregarPedido.dias =
          diasCab ?? diasInferidos ?? 1;
        ...
        this.lastDiasAplicado = this.normalizeDias(
          this.visualizarService.selectAgregarPedido.dias,
        );
        this.visualizarService.selectAgregarPedido.horasEstimadas =
        ...
        const servicioDesdeItems =
          this.selectedPaquetes.find((item) => item.servicioId != null)
            ?.servicioId ?? null;
        ...
        this.serviciosFechasSeleccionadas = serviciosFechas
          .map((entry) => {
            ...
            const itemTmpId =
              tmpIdMap.get(entry.idPedidoServicio) ??
              `i${entry.idPedidoServicio}`;
            return { itemTmpId, fecha };
          })
          .filter((entry) => entry.itemTmpId && entry.fecha);
        if (this.serviciosFechasSeleccionadas.length) {
          const tmpIds = new Set(
            this.selectedPaquetes.map((item) => item.tmpId),
          );
          this.serviciosFechasSeleccionadas =
            this.serviciosFechasSeleccionadas.filter((entry) =>
              tmpIds.has(entry.itemTmpId),
  ...
  updatePedido() {
    ...
    const erroresProgramacion =
      this.validarRestriccionesProgramacion(diasTrabajo);
    ...
    if (
      !this.ubicacion?.length ||
      !this.ubicacion.some((u) => (u?.Direccion || '').trim())
    ) {
      Swal.fire({
        icon: 'warning',
    ...
    const fechasBase = fechasUnicas.length
      ...
      : fechaEventoForm
        ...
        : [];
    const serviciosFechasAuto = (this.selectedPaquetes || []).flatMap(
      (item, index) => {
    ...
    const mensaje =
      (
        this.visualizarService.selectAgregarPedido as {
          mensaje?: string;
        } | null
      )?.mensaje ?? '';
    ...
    const serviciosFechasPayload = serviciosFechas
      .map((entry) => {
        ...
        const payloadEntry =
          idPedidoServicio != null
            ? {
                idPedidoServicio,
                itemTmpId: entry.itemTmpId,
                fecha: entry.fecha,
              }
    ...
    const payload: PedidoUpdatePayload = {
      pedido: {
        ...
        horasEstimadas:
          this.visualizarService.selectAgregarPedido?.horasEstimadas ?? null,
    ...
    const horaInvalida = payload.eventos.some(
      (e) => !/^\d{2}:\d{2}:\d{2}$/.test(e.hora),
    );
    const fechaInvalida = payload.eventos.some(
      (e) => !/^\d{4}-\d{2}-\d{2}$/.test(e.fecha),
    );
    ...
    const diff = this.getPedidoChanges(
      this.initialSnapshotData,
      currentSnapshotData,
    );
    if (diff.hasStrong) {
  ...
  private splitHoraParts(hora: string): {
    hora12: number | null;
    minuto: string | null;
    ampm: 'AM' | 'PM' | null;
  } {
  ...
  private ensureEventoSeleccionado(actionText: string): boolean {
    ...
    this.showAlert(
      ...
      `Primero selecciona el tipo de evento para ${actionText}.`,
    );
    return false;
  }
  ...
  private validarRestriccionesProgramacion(
    ...
    if (desordenadas) {
      errores.push(
        'Ordena las fechas de trabajo cronologicamente (de la mas proxima a la mas lejana).',
      );
    }
    for (const fecha of fechasTrabajo) {
      ...
      if (incompletas > 0) {
        errores.push(
          `"${fechaLabel}": tienes ${incompletas} locacion(es) con datos incompletos.`,
        );
      }
      if (Array.from(duplicadas.values()).some((total) => total > 1)) {
        errores.push(
          `"${fechaLabel}": hay locaciones duplicadas (misma locacion, direccion y hora).`,
        );
      }
    }
    return errores;
  ...
  private getPedidoChanges(
    ...
    if (
      viaticosClienteAntes !== viaticosClienteAhora ||
      viaticosMontoAntes !== viaticosMontoAhora
    ) {
      const before = viaticosClienteAntes
        ? 'Cliente cubre viaticos'
        : `Monto: ${viaticosMontoAntes}`;
      const after = viaticosClienteAhora
        ? 'Cliente cubre viaticos'
        : `Monto: ${viaticosMontoAhora}`;
      cambios.push({
        label: 'Viaticos',
        before,
        after,
    ...
    for (let i = 0; i < maxEventos; i += 1) {
      ...
      const cambioSoloNombre =
        JSON.stringify(prevSinNombre) === JSON.stringify(nextSinNombre) &&
        nombrePrev !== nombreNext;
      const cambioSoloNotas =
        JSON.stringify(prevSinNombreNotas) ===
          JSON.stringify(nextSinNombreNotas) &&
    ...
    const fechasAntesPorItem = this.buildFechasPorItemMap(
      anterior.serviciosFechas ?? [],
    );
    const fechasAhoraPorItem = this.buildFechasPorItemMap(
      actual.serviciosFechas ?? [],
    );
    ...
    allKeys.forEach((key) => {
      ...
      const cambiaronFechas =
        JSON.stringify(fechasPrev) !== JSON.stringify(fechasNext);
      if (cambiaronDatos || cambiaronFechas) {
        const label = prev.nombre || next.nombre || `Paquete ${key}`;
        const resumen = this.formatItemCambio(
          prev,
          next,
          fechasPrev,
          fechasNext,
    ...
    const prevNoMatch = anterior.items.filter(
      (item) => !matchedPrevKeys.has(String(item.key)),
    );
    const nextNoMatch = actual.items.filter(
      (item) => !matchedNextKeys.has(String(item.key)),
    );
    ...
    prevNoMatch.forEach((prev) => {
      ...
      const idx = nextPendientes.findIndex(
        (next) =>
      ...
      if (cambiaronDatos || cambiaronFechas) {
        const servicioLabel =
          this.getTipoServicioLabel(next) ||
          this.getTipoServicioLabel(prev) ||
          `Servicio ${prev.servicioId}`;
        const resumen = this.formatItemCambio(
          prev,
          next,
          fechasPrev,
    ...
    const prevSolo = anterior.items.filter(
      (item) => !matchedPrevKeys.has(String(item.key)),
    );
    prevSolo.forEach((prev) => {
      cambios.push({
        label: `${prev.nombre || `Paquete ${prev.key}`} (eliminado)`,
        before: this.formatItemSolo(
          prev,
          this.getFechasItem(fechasAntesPorItem, prev),
          false,
        ),
        after: '-',
        level: 'strong',
    ...
    const nextSolo = actual.items.filter(
      (item) => !matchedNextKeys.has(String(item.key)),
    );
    nextSolo.forEach((next) => {
      cambios.push({
        ...
        before: '-',
        after: this.formatItemSolo(
          next,
          this.getFechasItem(fechasAhoraPorItem, next),
          false,
        ),
        level: 'strong',
      });
    ...
    return {
      hasStrong: cambios.some((item) => item.level === 'strong'),
      items: cambios,
    };
  }
  ...
  private getEventoCambioLabel(
    prev?: PedidoEventoSnapshot,
    next?: PedidoEventoSnapshot,
    index = 1,
  ): string {
  ...
  private eventoSinNombreNotas(
    evento?: PedidoEventoSnapshot,
  ): PedidoEventoSnapshot {
    return {
      fecha: evento?.fecha ?? '',
      hora: evento?.hora ?? '',
      ubicacion: '',
  ...
  private formatItemSolo(
    item: PedidoItemSnapshot,
    fechas: string[],
  ...
  private sameNumber(a: unknown, b: unknown): boolean {
    ...
    const nb = Number(b ?? 0);
    return (
      Number.isFinite(na) && Number.isFinite(nb) && Math.abs(na - nb) < 0.000001
    );
  }
  ...
  private confirmStrongChanges(items: PedidoChangeItem[]): Promise<boolean> {
    const formatValue = (value: string) =>
      this.escapeHtml((value ?? '').trim() === '-' ? 'Sin registro' : value);
    const filas = items
      .map(
        (item) => `
